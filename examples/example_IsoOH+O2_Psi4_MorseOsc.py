...
if __name__ == '__main__':
    ...
    from utils.constants import BOHR_TO_ANGSTROM, CM1_TO_AU_ANGULAR_FREQUENCY, KJMOL_TO_HARTREE
    ...
    # r2SCAN-3c optimized O2 equilibrium data used to anchor the Morse well
    # to the old harmonic model near re.
    req_O2 = 1.21001  # Angstrom
    omega_O2 = 1610.19  # cm-1

    # Use a realistic O2 dissociation energy and derive beta so that the Morse
    # curvature at re reproduces the harmonic frequency:
    # omega = beta * sqrt(2 * De / mu)
    D0_O2 = 498.0  # kJ/mol
    omega_O2_au = omega_O2 * CM1_TO_AU_ANGULAR_FREQUENCY
    De_O2_au = D0_O2 * KJMOL_TO_HARTREE
    mass_O2 = get_mass_vector(['O', 'O'])
    redmass_O2 = mass_O2[0] * mass_O2[1] / (mass_O2[0] + mass_O2[1])
    beta_O2_au = omega_O2_au / math.sqrt(2.0 * De_O2_au / redmass_O2)
    beta_O2 = beta_O2_au / BOHR_TO_ANGSTROM  # convert Bohr^-1 to Angstrom^-1 input
    ...
    print("--------- O2 Morse Oscillator--------")
    print(f"O2 Morse parameters: re = {req_O2:.5f} Ang, D0 = {D0_O2:.1f} kJ/mol, beta = {beta_O2:.4f} Ang^-1")
    oxygen.print_mode_sampling()
    print("--------- O2 Morse Oscillator DONE--------")
    ...
    print("\nReaction of Trans-Isoprene-OH + O2 (Morse O2)")
    print()
