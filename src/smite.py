...
THERMOSTATS: tuple[str, ...] = ("berendsen", "andersen")
...
class Molecule:
    def __init__(
        ...
        if restart:
            ...
            if (
                len(mass) != len(atoms)
                or len(q_ini) != 3 * len(atoms)
                or len(p_ini) != 3 * len(atoms)
            ):
        ...
        self.vini = (
            None  # the initial (sampled) pot energy which is likely out of equilibrium
        )
        self.tini = None
        self.vsave = []
    ...
    def rotate_about_bond(self, atom1, atom2, theta, **kwargs):
        ...
        bond_th_HX = kwargs.get(
            "bond_th_HX", 1.4 / 0.5291772
        )  # bond threshold in Angstrom for H-X, where X = any non H-atom
        bond_th_XX = kwargs.get(
            "bond_th_XX", 2.0 / 0.5291772
        )  # bond threshold in Angstrom for X-X bonds to be considered as a part of a fragment

        self.q = rotate_fragment(
            self.atoms,
            self.q,
            atom_ax1=atom1,
            atom_ax2=atom2,
    ...
    def instantiate_propagator(
        self, integrator: str, integrator_order: int, tolerance
    ...
    def instantiate_quantum_propagator(
        self, q_integrator: str, de_cutoff: Optional[float] = None
    ) -> Optional[callable(object)]:
        q_propagators = {
        ...
        if thermostat is not None:
            if thermo_param or thermo_temp is None:
                ...
                if thermostat == "berendsen":
                    ...
                    self.p = thermo_berendsen(
                        self.nfix, self.p, self.wmass, dt, tau, thermo_temp
                    )
                if thermostat == "andersen":
                    self.p = thermo_andersen(
                        self.nfix, self.p, self.wmass, dt, thermo_param, thermo_temp
                    )

    def q_step(self, q_integrator, dt, dtq, q_propag):
        q_integrators = {"fssh": q_propag}
        if q_integrator is not None:
            if q_integrator in q_integrators:
                self.p, self.active_state = q_integrators[q_integrator](
                    dt,
                    self.active_state,
                    self.num_states,
                    self.q,
                ...
                raise ValueError(
                    "Non existing quantum integrator. You can choose from: fssh"
                )
    ...
    def run_trajectory(
        ...
        if pairs_to_stop is not None and Rstop is None:
            ...
            self.Rstop = (
                Rstop / b2a
            )  # Angstrom to Bohr #reactive event condition for trajectory
        else:
            raise ValueError(
        ...
        if not restart:
            ...
            if os.path.exists(traj_file):
                os.remove(traj_file)
                print(
                    f"{traj_file} already exisits, it has been deleted to create a new one"
                )
        ...
        with open(traj_file, "a") as file_trj:
            for istep in range(startstep, maxstep):
                # -----------------------------------------------------------------------------
                if (
                    iprint > 0 and istep % iprint == 0 and istep > startstep
                ) or istep == 0:
                    if self.qchem["wfu"]:
                        file_wf = os.path.join(
                            wf_dir,
                            "wavefunc_"
                            + self.fname
                            + "_step_"
                    ...
                    if collision:
                        ...
                        if self.pairstop is not None and self.Rstop is None:
                            tstop, channel = test_to_stop_specific(
                        ...
                        print(
                            f"step: {istep:<10d} t[fs]: {istep * dt / c6:>12.1f} V[Eh]: {(V - self.vref):<16.6f} E[Eh]: {(E - self.vref):<16.6f} dE[kJ]: {dE * c7:16.3f}    T[K]: {act_temp:10.1f}   Rcom[A]: {Rcom_actual * b2a:8.2f}",
                            flush=True,
                        )

                    else:  # if not collision (just unimolecular dynamics) then we can ran the test anytime
                        print(
                            f"step: {istep:<10d} t[fs]: {istep * dt / c6:<12.2f}  V[Eh]: {V:<16.6f} E[Eh]: {E:<16.6f} dE[kJ]: {dE * c7:16.3f}     T[K]: {act_temp:<10.1f}",
                            flush=True,
                        )
                        if self.pairstop is not None and self.Rstop is None:
                            tstop, channel = test_to_stop_specific(
                    ...
                    if tstop:
                        ...
                        formula = cluster_chemical_formulas(
                            self.q, self.atoms, eps=4.2, minPts=2
                        )
                ...
                if q_integrator is not None:
                    property_writer.write(
                        istep, V, dt, self.active_state, q_propag.c, q_propag.d
                    )
    ...
    def merge_with(self, other_molecule):
        if not isinstance(other_molecule, Molecule):
            raise ValueError(
                "other_fragment must be an instance of Molecule or Fragment"
            )
        ...
        new_fragment = Fragment(
            combined_atoms, combined_mass, combined_coord_eq, combined_mom
        )
    ...
    def print_trajectory(self, trajfile, istep, dt, T, V, dE, Temp):
        ...
        trajfile.write(str(self.natom) + "\n")
        trajfile.write(
            "%6s %10d %8s %10.2f %12s %16.8f %12s %12.8f %10s %15.4f %8s %7.1f\n"
            % (
                "step= ",
                istep,
                " t[fs]= ",
        ...
        for i in range(self.natom):
            ...
            jz = 3 * i + 2
            trajfile.write(
                "%3s %15.5f  %15.5f %15.5f  %16.6f  %16.6f %16.6f\n"
                % (
                    self.atoms[i],
                    self.q[jx] * b2a,
                    self.q[jy] * b2a,
...
class Fragment(Molecule):
    def __init__(
        ...
        q_ini,
        p_ini,
        constrained_bonds: Optional[list[list[int]]] = None,
        num_states: Optional[int] = 1,
        active_state: Optional[int] = 0,
    ):
        super().__init__(
            atoms=atoms,
            mass=mass,
    ...
    def Atom_Init(cls, fname, atoms, num_states: int = 1, active_state: int = 0):
        if len(atoms) != 1:
            raise ValueError(
                "ERROR: Atom_Init requires only a single atom. You must provide an array with a single element, for instance: ['Cl']"
            )
    ...
    def Diatom_Init(
        ...
        if req is None:
            ...
            raise ValueError(
                "ERROR: For non-rigid harmonic diatom req and omega must be given in Diatom_Init()"
            )
        elif (
            (req is None and alpha is None and De is None)
            and not rigid
            and diatom == "morse"
        ):
            raise ValueError(
                "ERROR: For non-rigid Morse diatom req, alpha, De must be given in Diatom_Init()"
            )
    ...
    def Polyatom_Init(
        ...
        Amp_modeanim=30.0,
        print_nmode=True,
        constrained_bonds: list[list[int]] = None,
        num_states: int = 1,
        active_state: int = 0,
    ):
    ...
    def Diatom_Sampling(self, **kwargs):
        if len(self.atoms) != 2:
            raise ValueError(
                "ERROR: DiatomHaromicInit accept only a diatomic molecule."
            )
        ...
        self.p, angmom, inertia = diatom_rotation_rigidrot_sampling(
            rot_modes=self.rotsampling, mass=self.mass, q=self.q, p=self.p
        )
    ...
    def Specify_Mode_Sampling(
        ...
        **kwargs,
        ...
        if not self.rigid and not self.fromMD and len(self.atoms) > 2:
            ...
            self.vibsampling = initialize_vibrational_modes(
                freq=self.freq, init_vib_type=init_vib_type, temp=temp
            )

            # if necessary then we may change certain modes sampling
            if {"fix_quantum", "fix_energy", "fix_temp"}.intersection(kwargs):
                self.vibsampling = specify_vib_modes(
                    vib_modes=self.vibsampling, **kwargs
                )
        ...
        self.rotsampling = initialize_rotational_modes(
            init_rot_type=init_rot_type, temp=temp, jrot=jrot
        )
    ...
    def print_mode_sampling(self):
        print(f"\n{self.fname}")
        print(
            "Vibrational mode sampling (Q: fixed quantum, E: fixed energy, T: thermal)"
        )
    ...
    def Polyatom_Sampling(self, **kwargs):
        ...
        if not self.rigid and not self.fromMD and not self.MDprimitive:
            self.q, self.p = polyatom_vibration_sampling(
                mass=self.mass,
                atoms=self.atoms,
                q_eq=self.q_ini,
                ww=self.freq,
        ...
        if (
            not self.MDprimitive
            and not self.surface
            and self.rotsampling[0][1] is not None
        ):
            self.p, angmom, inertia = polyatom_rotation_sampling(
                rot_modes=self.rotsampling, mass=self.mass, q=self.q, p=self.p
            )
...
class Collision(Molecule):
    def __init__(
        ...
        if constrained_bonds is None:
            if fragment_B.constrained_bonds is not None:
                fragment_B.constrained_bonds = [
                    [i + len(fragment_A.atoms), j + len(fragment_A.atoms)]
                    for i, j in fragment_B.constrained_bonds
                ]
                if fragment_A.constrained_bonds is None:
                    constrained_bonds = fragment_B.constrained_bonds
                else:
                    constrained_bonds = (
                        fragment_A.constrained_bonds + fragment_B.constrained_bonds
                    )
    ...
    def Specify_Collision_Sampling(
        self,
        Rini=None,
        bmax=None,
        bsampling=False,
        ...
        surf_skew_max=90.0,
        surf_skew_fix=None,
        surf_target_atom=None,
        surf_side=1,
    ):
        if (
            Rini == None
            or bmax == None
            or (Ecoll == None and Ecoll_thermal == False)
            or (Ecoll_thermal == True and temp == None)
        ):
            raise ValueError(
                "Rini, bmax and Ecoll (or Ecoll_thermal) must be give in the input of Specify_Collision_Sampling()"
            )
    ...
    def Set_Relative_Init_Coords_Molecule(self):
        ...
        if self.sampling_set == False:
            raise ValueError(
                "Error: First you must call Specify_Collision_Sampling() after you initialized the Collision() class"
            )
        ...
        if self.Ecoll_thermal:
            ...
            print(
                f"temp[K]: {self.tempcoll:<12.2f}     Ecoll[kJ/mol]: {self.Ecoll * 2625.5:<12.3f}"
            )

        if self.Ecoll == None or self.bmax == None or self.Rini == None:
            raise ValueError(
                "Ecoll (unless it's thermall sampled), bmax and Rini must be given in the input of Set_Relative_Init_Coords()"
            )
        ...
        if self.bsampling:
            ...
            print(
                f"Impact parameter has been randomly sampled: b = {self.bimp * 0.529177:>12.2f} Angstrom\n"
            )
        else:
            self.bimp = (
                self.bmax
            )  # fix impact parameter for opacity function P(b) calculations
    ...
    def Set_Relative_Init_Coords_Surface(self):
        ...
        if self.Rini < self.bmax:
            raise ValueError(
                "Error: Rini must be larger than b (impact paremter) for surface collisions"
            )
        ...
        if self.fragment_A.surface:
            ...
            qA, pA = orient_and_rotate_surface(
                surf_3atom, lab_axis, phi_yz_surf, qA, pA, self.fragment_A.mass
            )
        ...
        if self.fragment_B.surface:
            ...
            qB, pB = orient_and_rotate_surface(
                surf_3atom, lab_axis, phi_yz_surf, qB, pB, self.fragment_B.mass
            )
        # **************************************************************************************

        # Rini is the initial separation of center of masses of the aiming point and the center of mass of the projectile molecule
        # here the impact parameter (bimp) and the angle phi_yz together defines the aiming point on the surface

        # shift fragment B along the x-axis with sepx (separation along x-axis)
        # and shifted along the z-axis with bimp(impact paramter)

        if (
            self.fragment_B.surface and not self.fragment_A.surface
        ):  # in case of fragment-B is the surface then the smaller projectile is shifted
            print("Fragment B is the surface")
            print("theta_skew: ", theta_skew * 180.0 / math.pi)

            velA = math.sqrt(
                2.0 * self.Ecoll / self.fragment_A.totmass
            )  # only Fragment-A is flying toward the surface (Fragment-B)
            ...
            self.fragment_A.surface and not self.fragment_B.surface
        ):  # in case of a normal molecule-molecule scattering or when fragment-A is the surface:
            print("Fragment A is the surface")
            print("theta_skew: ", theta_skew * 180.0 / math.pi)

            velB = math.sqrt(
                2.0 * self.Ecoll / self.fragment_B.totmass
            )  # only Fragment-B is flying toward the surface (Fragment-A)
    ...
    def Sample_Bimolecular_Reactants(self, **kwargs):
        ...
        if self.overlap:
            raise ValueError(
                "Error: Overlap detected during the initialization of the bimolecular reaction!"
            )

        return

    def sample_and_run_collision(
        self,
        integrator="verlet",
        integrator_order=4,
        timestep=1.0,
        ...
        **kwargs,
        ...
        if backfile is None:
            backfile = (
                "backup_for_restart_of_reaction_"
                + self.fragment_A.fname
                + "_+_"
                + self.fragment_B.fname
                + ".xyz"
    ...
    def multi_onebyone_traj_sample_and_run_collision(
        self,
        ntraj=1,
        integrator="verlet",
        integrator_order=4,
