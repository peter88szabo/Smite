...
atomic_masses = {
    ...
    'V': 50.942,   'Cr': 51.996,  'Mn': 54.938,  'Fe': 55.845,  'Co': 58.933,  'Ni': 58.693,  'Cu': 63.546,  'Zn': 65.38,
    'Ga': 69.723,  'Ge': 72.63,   'As': 74.922,  'Se': 78.971,  'Br': 79.904,  'Kr': 83.798,  'Rb': 85.468,  'Sr': 87.62,
...
def get_mass_vector(atoms, **kwargs):
    ...
    for atom in atoms:
        if atom in atomic_masses:
            mass_vector.append(atomic_masses[atom] * gmol2au)
        else:
            mass_vector.append(None)  # In case an atomic symbol is not found
    return mass_vector

if __name__ == "__main__":
    atoms = ['H', 'H', 'O', 'C', 'N', 'Fe', 'O', 'H']
