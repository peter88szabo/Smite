...
def _rotatable_principal_axes(principal_moments, *, relative_tolerance=1.0e-10):
    """Return principal axes that correspond to realizable rigid rotations."""
    moments = np.asarray(principal_moments, dtype=float)
    if moments.shape != (3,) or not np.all(np.isfinite(moments)):
        raise ValueError("Principal moments must be three finite numbers")
    scale = max(1.0, float(np.max(np.abs(moments))))
    tolerance = float(relative_tolerance) * scale
    return np.flatnonzero(moments > tolerance)


def _sample_fixed_j_angular_momentum(jrot, principal_moments, principal_axes):
    """Sample a fixed-|J| vector in the physically rotatable subspace.

    A linear polyatomic molecule has one zero principal moment.  Angular
    momentum along that molecular axis cannot be generated by Cartesian
    rigid-body rotation, so its fixed-J distribution is uniform on the circle
    perpendicular to the axis, not on a three-dimensional sphere.
    """
    jrot = float(jrot)
    if not np.isfinite(jrot) or jrot < 0.0:
        raise ValueError("Fixed rotational quantum number jrot must be finite and non-negative")
    magnitude = math.sqrt(jrot * (jrot + 1.0))
    active = _rotatable_principal_axes(principal_moments)
    principal_axes = np.asarray(principal_axes, dtype=float)
    if principal_axes.shape != (3, 3):
        raise ValueError("principal_axes must have shape (3, 3)")
    ...
    if active.size == 2:
        ...
        components[active[0]] = magnitude * math.cos(angle)
        components[active[1]] = magnitude * math.sin(angle)
        return principal_axes @ components
    if magnitude == 0.0:
        return np.zeros(3, dtype=float)
    raise ValueError(
        "Fixed-J rotational sampling requires at least two non-zero principal moments; "
        "a point-like geometry cannot carry rigid-body angular momentum."
    )
...
def polyatom_rotation_sampling(rot_modes, mass, q, p):
    ...
    if sampling_mode == 'Q':
        ...
        jrot = excitation
        am = _sample_fixed_j_angular_momentum(jrot, Ixyz, principal_axes)
        if _rotatable_principal_axes(Ixyz).size == 2:
            print("Linear rotor detected: fixed-J direction sampled perpendicular to its molecular axis.")
