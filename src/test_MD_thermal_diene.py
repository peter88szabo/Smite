if __name__ == '__main__':
   ...
   #diene.Specify_Mode_Sampling(init_vib_type='ZPE', init_rot_type='Jfix', jrot=10, fix_quantum=fix_quantum, fix_temp=fix_temp)
    ...
    diene.Specify_Mode_Sampling(MDprimitive=True, temp=2000.0) # temp is for the intiail momentum selection

    print('\nNVT segment equilibriate at 500 K:')
    diene.sample_and_run_trajectory(traj_file='NVT_equilibriate.xyz',
                                    integrator='leapfrog',
                                    integrator_order=4,
                                    timestep=0.5,
                                    maxstep=1000,
