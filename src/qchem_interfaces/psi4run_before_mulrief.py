...
def Psi4_Force(q, atoms, qcinput):
   ...
   #Default values:
    ...
    # parse additional keywords (case-insensitive)
    ...
    # find maxiter=NN pattern (e.g., "maxiter=150" or "MAXITER=80")
    ...
    #ene = psi4.energy(functional)
    grad_matrix = psi4.gradient(functional)
    grad_matrix = np.array(grad_matrix)

    force = -grad_matrix.flatten(order='C')

    #grad, wfn = psi4.gradient("functional", return_wfn=True)
...
def Psi4_Energy(filename, q, atoms, qcinput):
   ...
   #Default values:
    ...
    opts = {
        ...
        "scf_type": "df",
        "maxiter": maxiter,
        "guess": "last",            # use last-step density
        "diis": True,
        "soscf": soscf,             # set True only if DIIS stalls
        "e_convergence": 1e-7 if tight else 1e-6,
        "d_convergence": 1e-7 if tight else 1e-6,
    ...
    if "scf_multiref" in add:
        # override guess strategy in multiref mode
        opts["guess"] = "sad"          # or leave this out and rely only on basis_guess
        opts["basis_guess"] = "sad"
        opts["diis_start"] = 2
        opts["damping_percentage"] = 30
        opts["level_shift"] = 0.3

    psi4.set_options(opts)

    ene, wfn = psi4.energy(functional, return_wfn=True)
