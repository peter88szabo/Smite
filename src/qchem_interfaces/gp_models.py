...
class GPModel:
    ...
    def transform_data(self, x: np.ndarray[float], state: int = None) -> np.ndarray[float]:
        """
    ...
    def mean(self, x: np.ndarray[float], state: int = None) -> np.ndarray[float]:
        """
    ...
    def variance(self, x: np.ndarray[float], state: int = None) -> np.ndarray[float]:
        """
    ...
    def grad(self, x: np.ndarray[float], state: int = None) -> np.ndarray[float]:
        """
        ...
        x = torch.autograd.Variable(torch.tensor(x, dtype=torch.float64), requires_grad=True)

        grad = torch.autograd.functional.jacobian(mean_f, x).squeeze()

        return grad.detach().numpy() * ev2ha

    def hess(self, x: np.ndarray[float], state: int = None) -> np.ndarray[float]:
        """
    ...
    def model(self, state: int = None) -> ScriptModule:
        """
    ...
    def _determine_state(self, state: int = None) -> int:
        """
        Helper function to determine the state if it is not specified.
        """
        if state is None:
            if self.n_states() > 1:
                raise NotImplementedError('The state must be specified if there are multiple states.')
            else:
                state = 0
